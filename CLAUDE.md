...
- ローカル開発インフラ設計（Docker Compose構成・環境変数・DBスキーマ反映方針）: `docs/domain/01_基本設計/07_基盤設計.md`
- ブランチ戦略（GitHub Flow運用・PRルール）: `docs/domain/01_基本設計/08_ブランチ戦略.md`
...
- `main` へ直接pushしない。作業は `feature/<機能名>` ブランチで行い、全テストが成功した状態でPRを作成する（詳細: `docs/domain/01_基本設計/08_ブランチ戦略.md`）。
