...
| エディタ連携 | `docker compose up`（`./scripts/up.sh`）に加え、VS Code Dev Containers用の `.devcontainer/` を用意する（2026-08-11追記、下記「Devcontainer」節参照）。作成当初は「作らない」方針だったが、リモートSSHサーバ上での開発ニーズにより方針転換した |
| Webの状態管理 | Pinia |
...
## Devcontainer（リモートSSHサーバでの開発、2026-08-11追記）

作業を自宅サーバ等のリモートSSH先で行えるようにするため、`.devcontainer/` を追加した（`mobile` はXcode/macOSネイティブ必須のため対象外。`api`/`web`が対象）。

- 接続方式: VS Codeの「Remote - SSH」拡張機能でリモートサーバへ接続し、リポジトリを開いたうえで「Dev Containers: Reopen in Container」を実行する（Remote-SSH + Dev Containersの組み合わせ）。リモートサーバ側にDockerがインストール・起動済みであることが前提。SSH接続先の設定（`~/.ssh/config`等）はVS Code側のローカル設定であり、リポジトリには含めない。
- 構成: 既存の `docker-compose.yml` をそのまま使い、`.devcontainer/docker-compose.yml` で `api` サービスにリポジトリルート全体を `/workspace` として追加マウントするだけの薄い上書きに留める（`api` サービス自体の起動コマンド・環境変数は変更しない）。VS Codeは `api` サービスにアタッチし、`workspaceFolder=/workspace` からリポジトリ全体（`api`/`web`/`mobile`/`docs`等）を編集できる。
- devcontainer起動時は他サービス（db/minio/web）も通常の `docker compose up` 同様に立ち上がるため、`web` のVite dev serverはアタッチ先とは別の `web` コンテナ内でホットリロードしたまま動作し続ける。
- `api/Dockerfile` にはNode.js 20 + pnpm（`corepack prepare pnpm@10.15.1`、`web`と同一バージョンに固定）も同居させており、アタッチ先の統合ターミナル1つから `./gradlew` と `pnpm` の両方を実行できる。
- `shutdownAction` は `none` とし、VS Codeの接続が切れてもコンテナ群は停止しない（複数人・複数セッションで共有しうるリモートサーバのため、意図せず他の作業を止めないようにする）。
- `api` サービスにはClaude Code CLI（`@anthropic-ai/claude-code`）も導入済み。認証情報は `CLAUDE_CONFIG_DIR=/root/.claude` を named volume（`claude-config`）にマウントすることでコンテナ再作成後も保持される。初回のみ統合ターミナルで `claude` を実行しログインする（表示されるURLをローカルのブラウザで開いて認証を完了する、SSH越しでも動作するフロー）。
